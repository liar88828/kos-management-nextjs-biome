        ...
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center justify-between">
                    ...
                    <Badge variant={ room.status === "kosong" ? "default" : room.status === "terisi" ? "secondary" : "destructive" }>{ room.status }</Badge>
                </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                { room.foto && (
                    <picture>
                        <img alt={ `Kamar ${ room.nomor }` } className="w-full rounded-lg object-cover" src={ room.foto }/>
                    </picture>
                ) }
